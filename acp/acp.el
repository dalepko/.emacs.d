...
(defcustom acp-agent-command '("opencode" "acp")
...
(defvar-local acp--agent nil)
(defvar-local acp--markdown-buffer nil)
(defvar-local acp--pending-permission-requests nil)
(defvar-local acp--plan-widget nil)
(defvar-local acp--prompt nil)
(defvar-local acp--response-begin nil)
(defvar-local acp--snapshot-before nil)
(defvar-local acp--tool-widgets nil)

(defvar-local acp--captured-context nil
...
(define-derived-mode acp-mode fundamental-mode "ACP"
  ...
  (setq acp--agent
        (acp-agent-start
         acp-agent-command))
  (acp-agent-add-hook acp--agent :on-stop #'acp--on-stop)
  (acp-agent-add-hook acp--agent :on-message-chunk #'acp--on-message-chunk)
  (acp-agent-add-hook acp--agent :on-tool-call #'acp--on-tool-call)
  (acp-agent-add-hook acp--agent :on-tool-call-update #'acp--on-tool-call-update)
  (acp-agent-add-hook acp--agent :on-plan #'acp--on-plan)
  (acp-agent-add-hook acp--agent :on-prompt-done #'acp--on-prompt-done)
  (acp-agent-add-hook acp--agent :on-permission-request #'acp--on-permission-request)
  (acp-agent-add-hook acp--agent :on-error #'acp--on-error)
  (acp-modeline-config-options-setup acp--agent)
  (acp-modeline-usage-setup acp--agent))
