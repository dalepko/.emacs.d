...
(defun acp-permission-widget--value-create (widget)
  ...
  (let* ((pr (widget-get widget :value))
         ...
         (options (acp-permission-request-options pr))
         (title (or (when-let* ((content (acp-tool-call-content tool-call))
                                (text-item (seq-find (lambda (c)
                                                       (and (acp-tool-call-content-p c)
                                                            (equal (acp-tool-call-content-type c) "text")))
                                                     content)))
                      (acp-tool-call-content-text text-item))
                    (acp-tool-call-title tool-call)))
